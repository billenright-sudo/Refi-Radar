...
const SAMPLE_CLIENTS = [
  { id: 1, name: "Robert & Linda Chen", email: "rchen@email.com", phone: "(555) 010-1234", loanType: "30yr_fixed", currentRate: 7.85, loanBalance: 485000, propertyValue: 650000, closeDate: "2022-08-15", creditScore: 760, notes: "Referred by James at Title Co." },
  { id: 2, name: "Marcus Williams", email: "mwilliams@email.com", phone: "(555) 010-2345", loanType: "30yr_fixed", currentRate: 7.25, loanBalance: 320000, propertyValue: 420000, closeDate: "2023-02-20", creditScore: 740, notes: "" },
  { id: 3, name: "Sarah & Tom Patel", email: "spatel@email.com", phone: "(555) 010-3456", loanType: "15yr_fixed", currentRate: 6.90, loanBalance: 215000, propertyValue: 380000, closeDate: "2023-06-10", creditScore: 800, notes: "Self-employed - may need extra docs" },
  { id: 4, name: "Jennifer & Carlos Lopez", email: "jlopez@email.com", phone: "(555) 010-4567", loanType: "5_1_arm", currentRate: 8.25, loanBalance: 560000, propertyValue: 720000, closeDate: "2022-11-30", creditScore: 720, notes: "ARM adjusting soon - high priority" },
  { id: 5, name: "David & Amy Kowalski", email: "dkowalski@email.com", phone: "(555) 010-5678", loanType: "30yr_fixed", currentRate: 6.50, loanBalance: 185000, propertyValue: 290000, closeDate: "2023-09-05", creditScore: 785, notes: "" },
  { id: 6, name: "Michael Thompson", email: "mthompson@email.com", phone: "(555) 010-6789", loanType: "30yr_fixed", currentRate: 8.10, loanBalance: 425000, propertyValue: 580000, closeDate: "2022-06-18", creditScore: 755, notes: "Phone contact only" },
  { id: 7, name: "Aisha Johnson", email: "ajohnson@email.com", phone: "(555) 010-7890", loanType: "15yr_fixed", currentRate: 7.40, loanBalance: 178000, propertyValue: 310000, closeDate: "2023-01-12", creditScore: 810, notes: "" },
  { id: 8, name: "Carlos & Maria Reyes", email: "creyes@email.com", phone: "(555) 010-8901", loanType: "30yr_fixed", currentRate: 7.65, loanBalance: 390000, propertyValue: 510000, closeDate: "2022-09-22", creditScore: 748, notes: "Interested in cash-out refi" },
];
...
const inputStyle = { background: C.surfaceHi, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 14px", color: C.text, fontFamily: "inherit", fontSize: 14, width: "100%", outline: "none" };
...
const TreasuryWidget = ({ treasury }) => {
  ...
  return (
    <div style={{ ...card, background: "#0d0a1f", borderColor: "#2a1f4a", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
      <div>
        <div style={{ color: C.purple, fontSize: 11, fontWeight: 700, letterSpacing: "0.5px", textTransform: "uppercase", marginBottom: 6 }}>
        ...
        <div style={{ color: C.muted, fontSize: 12, marginTop: 4 }}>
          {current?.date} · Federal Reserve (FRED)
        </div>
      </div>
...
const Dashboard = ({ scored, refiReady, rates, ratesLoading, totalSavings, fetchRates, setView, setSelected, treasury }) => {
  ...
  return (
    <div style={{ animation: "fadeIn 0.35s ease" }}>
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "2rem" }}>
        <StatCard label="Refi Opportunities" value={ratesLoading ? "—" : refiReady.length} sub="clients ready today" accent={refiReady.length > 0 ? C.amber : C.muted} />
        <StatCard label="Potential Monthly Savings" value={ratesLoading ? "—" : $c(totalSavings / 12)} sub="across all opportunities" accent={C.green} />
        <StatCard label="Total Portfolio" value={scored.length} sub="clients tracked" />
      ...
      {rates && !ratesLoading && (
        <div style={{ ...card, display: "flex", gap: "2rem", alignItems: "center", marginBottom: "2rem", background: C.amberBg, borderColor: "#3a2800" }}>
          <div style={{ color: C.amber, fontSize: 12, fontWeight: 700, letterSpacing: "0.5px", display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: rates.source === "Estimated" ? C.muted : C.green, display: "inline-block" }} />
            {rates.source === "Estimated" ? "ESTIMATED RATES" : "LIVE RATES"}
          </div>
      ...
      <div>
        ...
        {ratesLoading ? (
          <div style={{ ...card, textAlign: "center", padding: "3rem", color: C.muted }}>Analyzing portfolio with live rates...</div>
        ) : topOpps.length === 0 ? (
...
const Clients = ({ scored, setSelected, setView }) => {
  ...
  const filtered = scored.filter(c => filter === "all" ? true : filter === "refi" ? c.a?.good : !c.a?.good);
  return (
    <div style={{ animation: "fadeIn 0.35s ease" }}>
...
const ClientDetail = ({ client, rates, setView }) => {
    ...
    <div style={{ animation: "fadeIn 0.3s ease", maxWidth: 720 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.5rem" }}>
        <div>
          ...
          <div style={{ color: C.muted, fontSize: 14 }}>Closed {new Date(client.closeDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</div>
        </div>
      ...
      <div style={{ ...card, marginBottom: "1.5rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1rem" }}>
          {[["Loan Type", LOAN_LABELS[client.loanType]], ["Current Rate", $r(client.currentRate)], ["Remaining Balance", $c(client.loanBalance)], ["Property Value", $c(client.propertyValue)], ["LTV", a ? `${a.ltv.toFixed(1)}%` : "—"], ["Est. Monthly Pmt", a ? $c(a.curPmt) : "—"]].map(([label, val]) => (
            <div key={label}>
...
const RateWatch = ({ rates, ratesLoading, fetchRates, lastFetched, ratesError, treasury }) => (
  <div style={{ animation: "fadeIn 0.35s ease", maxWidth: 640 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
      ...
      <button onClick={fetchRates} disabled={ratesLoading} style={{ display: "flex", alignItems: "center", gap: 8, background: C.amber, color: "#000", border: "none", borderRadius: 8, padding: "9px 16px", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit", opacity: ratesLoading ? 0.6 : 1 }}>
      ...
      <div style={{ ...card, background: C.amberBg, borderColor: "#3a2800" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "1rem" }}>
          <span style={{ fontSize: 12, color: C.amber, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px" }}>
            {rates.source === "Estimated" ? "Estimated Rates" : "Live Mortgage Rates"}
          </span>
...
const AddClient = ({ setClients, setView }) => {
  ...
  const submit = () => {
    ...
    setClients(prev => [...prev, { ...form, id: Date.now(), currentRate: +form.currentRate, loanBalance: +form.loanBalance, propertyValue: +form.propertyValue, creditScore: +form.creditScore || 700 }]);
    setSaved(true);
    setTimeout(() => { setForm(EMPTY_FORM); setSaved(false); setView("clients"); }, 1500);
  };
  ...
  return (
    <div style={{ animation: "fadeIn 0.35s ease", maxWidth: 620 }}>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: C.text, marginBottom: "1.5rem" }}>Add New Client</h1>
      {saved && <div style={{ ...card, background: C.greenBg, borderColor: "#0f4a25", color: C.green, marginBottom: "1rem", display: "flex", gap: 10, alignItems: "center" }}><Icon name="check" size={16} color={C.green} /> Client saved!</div>}
      <div style={{ ...card }}>
        ...
        <div style={{ display: "flex", gap: "1rem" }}>
          <button onClick={submit} style={{ background: C.amber, color: "#000", border: "none", borderRadius: 8, padding: "11px 24px", fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>Save Client</button>
          <button onClick={() => setView("clients")} style={{ background: "transparent", color: C.muted, border: `1px solid ${C.border}`, borderRadius: 8, padding: "11px 20px", fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
        </div>
      </div>
    </div>
  );
  ...
  const fetchRates = useCallback(async () => {
    ...
    try {
      ...
      if (data.rates) {
        ...
        if (!data.success) setRatesError("Could not fetch live rates");
      }
    } catch (err) {
      setRatesError("Server error - showing estimated rates");
      setRates({ rate_30yr_fixed: 6.87, rate_15yr_fixed: 6.18, rate_5_1_arm: 6.52, date: new Date().toISOString().split("T")[0], source: "Estimated" });
      setLastFetched(new Date());
    }
    ...
    <div style={{ fontFamily: "'Sora', 'Segoe UI', sans-serif", background: C.bg, minHeight: "100vh", color: C.text }}>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <main style={{ flex: 1, padding: "2rem 2.5rem", overflowX: "hidden" }}>
          {view === "dashboard" && <Dashboard scored={scored} refiReady={refiReady} rates={rates} ratesLoading={ratesLoading} totalSavings={totalSavings} fetchRates={fetchRates} setView={setView} setSelected={setSelected} treasury={treasury} />}
          ...
          {view === "client-detail" && selected && <ClientDetail client={selected} rates={rates} setView={setView} />}
        </main>
      </div>
    </div>
  );
